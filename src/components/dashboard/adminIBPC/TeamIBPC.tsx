...
import SideBarSection from "./sidebarIBPC";
import {
	useGetIBPCTeams,
	useUpdateIBPCTeamStatus,
...
const TeamIBPC = () => {
	...
	const { updateTeamStatus } = useUpdateIBPCTeamStatus();
	const [isChangingStatus, setIsChangingStatus] = useState<Set<number>>(
		new Set()
	);
	const { data } = useGetIBPCTeams();
	useEffect(() => {
		if (data) {
			setTeams(data);
		}
