...
const ICData = () => {
  ...
  const [currentPage, setCurrentPage] = useState('international-conference');
  ...
  return (
    <div className="min-h-screen">
      {/* Header */}
      <header className="relative sm:px-6 py-4">
                  ...
                  filteredData.map((participant, index) => (
                    <tr key={participant.no} className={index % 2 === 0 ? "bg-[#8257A9]" : "bg-[#8257b8]"}>
                      ...
                      <td className="py-3 px-4">
                        <button
                          onClick={() => handlePreview(index)}
                          className="flex items-center space-x-1 bg-purple-800 cursor-pointer hover:bg-purple-900 px-2 py-1 rounded text-white text-sm transition-colors"
                        >
                          <Eye className="w-3 h-3" />
                          <span>Preview</span>
                        </button>
